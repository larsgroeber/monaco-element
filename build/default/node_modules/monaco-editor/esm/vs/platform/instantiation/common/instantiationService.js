...
function () {
  ...
  InstantiationService.prototype._createInstance = function (desc, args) {
    ...
    var serviceDependencies = _util.getServiceDependencies(desc.ctor).sort(function (a, b) {
    ...
    var firstServiceArgPos = serviceDependencies.length > 0 ? serviceDependencies[0].index : staticArgs.length; // check for argument mismatches, adjust static args if needed
  ...
  InstantiationService.prototype._createAndCacheServiceInstance = function (id, desc) {
    ...
    while (stack.length) {
      var item = stack.pop();
      graph.lookupOrInsertNode(item); // TODO@joh use the graph to find a cycle
    ...
    while (true) {
      ...
      for (var _a = 0, roots_1 = roots; _a < roots_1.length; _a++) {
        var root = roots_1[_a]; // create instance and overwrite the service collections
