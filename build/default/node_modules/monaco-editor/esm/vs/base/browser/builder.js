...
import * as DOM from './dom.js'; // --- Implementation starts here
...
function () {
  ...
  Builder.prototype.build = function (container, index) {
    assert.ok(this.offdom, 'This builder was not created off-dom, so build() can not be called.'); // Use builders own container if present
    ...
    var htmlContainer = container; // Append
  ...
  Builder.prototype.appendTo = function (container, index) {
    ...
    assert.ok(container, 'Builder can only be build() with a container provided.');
    assert.ok(DOM.isHTMLElement(container), 'The container must either be a HTMLElement or a Builder.');
    var htmlContainer = container; // Remove node from parent, if needed
  ...
  Builder.prototype.doElement = function (name, attributesOrFn, fn) {
    ...
    this.currentElement = element; // Off-DOM: Remember in array of created elements
  ...
  Builder.prototype.on = function (arg1, fn, listenerToDisposeContainer, useCapture) {
    var _this = this; // Event Type Array
    ...
    if (types.isArray(arg1)) {
      arg1.forEach(function (type) {
        ...
        this.setProperty(LISTENER_BINDING_ID, listenerBinding); // Add to Array if passed in
  ...
  Builder.prototype.style = function (firstP, secondP) {
    ...
    var hasFirstP = types.isString(firstP); // Get Style Value
  ...
  Builder.prototype.show = function () {
    ...
    this.attr('aria-hidden', 'false'); // Cancel any pending showDelayed() invocation
  ...
  Builder.prototype.hide = function () {
    ...
    this.attr('aria-hidden', 'true'); // Cancel any pending showDelayed() invocation
  ...
  Builder.prototype.unbindDescendants = function (current) {
    if (current && current.children) {
      for (var i = 0, length_1 = current.children.length; i < length_1; i++) {
        var element = current.children.item(i); // Unbind
...
function (_super) {
  ...
  function MultiBuilder(builders) {
    ...
    _this.length = 0;
    _this.builders = []; // Add Builders to Array
    ...
    var propertyFn = function (prop) {
      $outer[prop] = function () {
        ...
        for (var i = 0; i < $outer.length; i++) {
          var res = $outer.item(i)[prop].apply($outer.item(i), args); // Merge MultiBuilders into one
