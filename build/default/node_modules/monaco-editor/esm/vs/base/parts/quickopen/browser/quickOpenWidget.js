...
function (_super) {
  ...
  QuickOpenWidget.prototype.create = function () {
    ...
    this.builder = $().div(function (div) {
      ...
      }).on(DOM.EventType.CONTEXT_MENU, function (e) {
        return DOM.EventHelper.stop(e, true);
      }) // Do this to fix an issue on Mac where the menu goes into the way
      .on(DOM.EventType.FOCUS, function (e) {
        return _this.gainingFocus();
      }, null, true).on(DOM.EventType.BLUR, function (e) {
        return _this.loosingFocus(e);
      }, null, true); // Progress Bar

      _this.progressBar = _this._register(new ProgressBar(div.clone(), {
        progressBarBackground: _this.styles.progressBarBackground
      }));

      _this.progressBar.hide(); // Input Field
      ...
      _this.treeContainer = div.div({
        ...
        var createTree = _this.options.treeCreator || function (container, config, opts) {
        ...
        _this.treeElement = _this.tree.getHTMLElement(); // Handle Focus and Selection event
  ...
  QuickOpenWidget.prototype.show = function (param, options) {
    ...
    this.quickNavigateConfiguration = options ? options.quickNavigateConfiguration : void 0; // Adjust UI for quick navigate mode
  ...
  QuickOpenWidget.prototype.setInputAndLayout = function (input, autoFocus) {
    ...
    this.treeContainer.style({
      height: this.getHeight(input) + "px"
    });
  ...
  QuickOpenWidget.prototype.updateResultCount = function (count) {
    this.resultCount.text(nls.localize({
  ...
  QuickOpenWidget.prototype.hide = function (reason) {
    ...
    this.visible = false;
    this.builder.hide();
    this.builder.domBlur(); // Clear input field and clear tree
    ...
    this.treeContainer.style({
      height: (this.options.minItemsToShow ? this.options.minItemsToShow * 22 : 0) + 'px'
