...
import { LRUCache } from './map.js'; // Combined filters
...
export function fuzzyScore(pattern, word, patternMaxWhitespaceIgnore, firstMatchCanBeWeak) {
  ...
  var wordLen = word.length > 100 ? 100 : word.length; // Check for leading whitespace in the pattern and
  ...
  var wordPos = 0; // Run a simple check if the characters of pattern occur
...
function _findAllMatches(patternPos, wordPos, total, matches, lastMatched) {
  ...
  while (patternPos > _patternStartPos && wordPos > 0) {
    ...
    /* Diag */
        ...
        lastMatched = true; // count simple matches and boost a row of
