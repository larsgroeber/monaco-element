...
export function normalize(path, toOSPath) {
  ...
  var root = getRoot(path, sep); // skip the root-portion of the path
  ...
  for (var end = root.length; end <= len; end++) {
    ...
    if (end === len || path.charCodeAt(end) === 47
    /* Slash */
    || path.charCodeAt(end) === 92
    /* Backslash */
    ) {
...
export function isAbsolute(path) {
  return isWindows ? isAbsolute_win32(path) : isAbsolute_posix(path);
}
...
export function isAbsolute_posix(path) {
  return path && path.charCodeAt(0) === 47
  /* Slash */
  ;
}