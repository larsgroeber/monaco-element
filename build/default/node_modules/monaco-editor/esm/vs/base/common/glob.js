    ...
    regEx = '.*';
      ...
      segments.forEach(function (segment, index) {
        ...
        for (var i = 0; i < segment.length; i++) {
          char = segment[i]; // Support brace expansion
  ...
  if (value) {
    ...
    if (typeof when_1 === 'string') {
      var result = function (path, basename, name, hasSibling) {
        ...
        return TPromise.is(matched) ? matched.then(function (m) {
...
function aggregateBasenameMatches(parsedPatterns, result) {
  ...
  var basenames = basenamePatterns.reduce(function (all, current) {
    return all.concat(current.basenames);
  }, []);
  var patterns;
  ...
  if (result) {
    ...
    patterns = basenamePatterns.reduce(function (all, current) {
      return all.concat(current.patterns);
    }, []);
  }
