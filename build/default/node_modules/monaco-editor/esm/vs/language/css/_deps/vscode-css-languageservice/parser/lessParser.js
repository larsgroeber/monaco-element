...
function (_super) {
  ...
  LESSParser.prototype._parseStylesheetStatement = function () {
    if (this.peek(TokenType.AtKeyword)) {
      return this._parseVariableDeclaration() || this._parsePlugin() || _super.prototype._parseStylesheetAtStatement.call(this);
    }

    return this._tryParseMixinDeclaration() || this._tryParseMixinReference(true) || this._parseRuleset(true);
  };
  ...
  LESSParser.prototype._parseMediaDeclaration = function (isNested) {
    ...
    return this._tryParseRuleset(isNested) || this._tryToParseDeclaration() || this._tryParseMixinDeclaration() || this._tryParseMixinReference() || this._parseDetachedRuleSetMixin() || this._parseStylesheetStatement();
  };
  ...
  LESSParser.prototype._parseTerm = function () {
    ...
    if (term.setExpression(this._parseVariable()) || term.setExpression(this._parseEscaped())) {
  ...
  LESSParser.prototype._acceptInterpolatedIdent = function (node) {
    ...
    while (this.accept(TokenType.Ident) || node.addChild(this._parseInterpolation() || this.try(delimWithInterpolation))) {
      ...
      if (!this.hasWhitespace() && this.acceptDelim('-')) {// '-' is a valid char inside a ident (special treatment here to support @{foo}-@{bar})
      }
...
export { LESSParser }; //# sourceMappingURL=lessParser.js.map