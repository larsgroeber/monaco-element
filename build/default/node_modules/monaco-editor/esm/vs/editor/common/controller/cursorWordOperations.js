...
function () {
  ...
  WordOperations.moveWordLeft = function (wordSeparators, model, position, wordNavigationType) {
    ...
    /* WordStart */
        ...
        /* Separator */
            ...
            if (prevWordOnLine.end - prevWordOnLine.start === 1 && prevWordOnLine.nextCharClass === 0
            /* Regular */
            ) {
  ...
  WordOperations.word = function (config, model, cursor, inSelectionMode, position) {
    ...
    if (!inSelectionMode) {
      ...
      var isTouchingPrevWord = prevWord && prevWord.wordType === 1
      /* Regular */
      && prevWord.start <= position.column - 1 && position.column - 1 <= prevWord.end;
      var isTouchingNextWord = nextWord && nextWord.wordType === 1
      /* Regular */
    ...
    var isInsidePrevWord = prevWord && prevWord.wordType === 1
    /* Regular */
    && prevWord.start < position.column - 1 && position.column - 1 < prevWord.end;
    var isInsideNextWord = nextWord && nextWord.wordType === 1
    /* Regular */
    && nextWord.start < position.column - 1 && position.column - 1 < nextWord.end;
    var startColumn;
    var endColumn;
...
export function _lastWordPartEnd(str, startIndex) {
  ...
  for (var i = startIndex; i >= 0; i--) {
    ...
    || chCode === 9
    /* Tab */
    || !ignoreUpperCase && strings.isUpperAsciiLetter(chCode) || chCode === 95
    /* Underline */
    ) {
        return i - 1;
      }
...
export function _nextWordPartBegin(str, startIndex) {
  ...
  var chCode = str.charCodeAt(startIndex); // handle the special case ' X' and ' x' which is different from the standard methods

  if ((prevChCode === 32
  /* Space */
  || prevChCode === 9
  /* Tab */
  ) && (strings.isLowerAsciiLetter(chCode) || strings.isUpperAsciiLetter(chCode))) {
    return startIndex + 1;
  }
  ...
  for (var i = startIndex; i < str.length; ++i) {
    ...
    || chCode === 9
    /* Tab */
    || !ignoreUpperCase && strings.isUpperAsciiLetter(chCode)) {
      return i + 1;
    }
