...
function () {
  ...
  TypeOperations._isAutoClosingOpenCharType = function (config, model, selections, ch) {
    ...
    for (var i = 0, len = selections.length; i < len; i++) {
      ...
      var position = selection.getPosition();
      var lineText = model.getLineContent(position.lineNumber); // Do not auto-close ' or " after a word character
  ...
  TypeOperations.compositionEndWithInterceptors = function (prevEditOperationType, config, model, selections) {
    ...
    for (var i = 0; i < selections.length; i++) {
      ...
      if (config.autoClosingPairsClose.hasOwnProperty(ch)) {
        // first of all, it's a closing tag
        if (ch === config.autoClosingPairsClose[ch]
        /** isEqualPair */
        ) {
