    ...
    if (element instanceof ReferencesModel) {
      ...
      return element.resolve(this._textModelResolverService).then(function (val) {
        if (element.failure) {
          ...
          return tree.refresh(element).then(function () {
            return val.children;
          });
        }
...
function () {
  ...
  FileReferencesTemplate = __decorate([__param(1, IWorkspaceContextService), __param(2, optional(IEnvironmentService)), __param(3, IThemeService)], FileReferencesTemplate);
...
function () {
  ...
  OneReferenceTemplate.prototype.set = function (element) {
    var _a = element.parent.preview.preview(element.range),
        before = _a.before,
        inside = _a.inside,
        after = _a.after;
...
function () {
  ...
  Renderer = __decorate([__param(0, IWorkspaceContextService), __param(1, IThemeService), __param(2, optional(IEnvironmentService))], Renderer);
  return Renderer;
}();
  ...
  function VSash(container, ratio) {
    ...
    this._sash = new Sash(container, {
      getVerticalSashLeft: function () {
        return _this._width * _this._ratio;
      },
      getVerticalSashHeight: function () {
...
function (_super) {
  ...
  ReferenceWidget.prototype._fillBody = function (containerElement) {
    ...
    this._sash.onDidChangePercentages(function () {
      var _a = _this._sash.percentages,
          left = _a[0],
          right = _a[1];

      _this._previewContainer.style({
        width: left
      });

      _this._treeContainer.style({
        width: right
      });
    ...
    container.div({
      ...
      var controller = _this._instantiationService.createInstance(Controller, {
        keyboardSupport: _this._defaultTreeKeyboardSupport,
        clickBehavior: ClickBehavior.ON_MOUSE_UP
        /* our controller already deals with this */
      ...
      ctxReferenceWidgetSearchTreeFocused.bindTo(_this._tree.contextKeyService); // listen on selection and focus
      ...
      _this._disposables.push(controller.onDidFocus(function (element) {
        return onEvent(element, 'show');
      }));

      _this._disposables.push(controller.onDidSelect(function (element) {
        return onEvent(element, 'goto');
      }));

      _this._disposables.push(controller.onDidOpenToSide(function (element) {
        return onEvent(element, 'side');
      }));
  ...
  ReferenceWidget.prototype._doLayoutBody = function (heightInPixel, widthInPixel) {
    ...
    this._sash.width = widthInPixel; // set height/width

    var _a = this._sash.percentages,
        left = _a[0],
        right = _a[1];
    ...
    this._preview.layout(); // store layout data
  ...
  ReferenceWidget.prototype._onNewModel = function () {
    ...
    this._disposeOnNewModel.push(this._model.onDidChangeReferenceRange(function (reference) {
      return _this._tree.refresh(reference);
    })); // listen on editor
    ...
    this._disposeOnNewModel.push(this._preview.onMouseDown(function (e) {
      ...
      if (event.detail === 2) {
        _this._onDidSelectReference.fire({
          element: {
            uri: _this._getFocusedReference().uri,
            range: target.range
          },
      ...
      return __generator(this, function (_a) {
        switch (_a.label) {
          ...
          case 2:
            return [2
            /*return*/
            , TPromise.join([promise, this._tree.reveal(reference)]).then(function (values) {
              var ref = values[0];

              if (!_this._model) {
                ref.dispose(); // disposed

                return;
              }

              dispose(_this._previewModelReference); // show in editor
              ...
              if (model) {
                ...
                _this._preview.revealRangeInCenter(sel, isSameModel ? 0
                /* Smooth */
                : 1
                /* Immediate */
                );
  ...
  ReferenceWidget = __decorate([__param(3, IThemeService), __param(4, ITextModelService), __param(5, IInstantiationService), __param(6, IUriDisplayService)], ReferenceWidget);
