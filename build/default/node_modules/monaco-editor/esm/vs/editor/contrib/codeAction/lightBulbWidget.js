...
function () {
  function LightBulbWidget(editor) {
    ...
    this._disposables.push(this._editor.onDidChangeModel(function (_) {
      return _this._futureFixes.cancel();
    }));

    this._disposables.push(this._editor.onDidChangeModelLanguage(function (_) {
      return _this._futureFixes.cancel();
    }));
    ...
    set: function (value) {
      ...
      this._model.actions.then(function (fixes) {
        if (!token.isCancellationRequested && fixes && fixes.length > 0) {
          if (selection.isEmpty() && fixes.every(function (fix) {
            return fix.kind && CodeActionKind.Refactor.contains(fix.kind);
          })) {
            _this.hide();
          } else {
            _this._show();
