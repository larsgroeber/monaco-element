...
function () {
  ...
  TokenScanner.prototype.next = function () {
    ...
    if (!modeBrackets || ignoreBracketsInToken(tokenType)) {
      return new Token(new Range(lineNumber, startOffset + 1, lineNumber, endOffset + 1), 0
      /* None */
      , null);
    }
    ...
    do {
      ...
      if (foundBracket) {
        ...
        if (startOffset < foundBracketStartOffset) {
          ...
          this._nextBuff.push(new Token(new Range(lineNumber, startOffset + 1, lineNumber, foundBracketStartOffset + 1), 0
          /* None */
          , null));
        }
        ...
        this._nextBuff.push(new Token(new Range(lineNumber, foundBracketStartOffset + 1, lineNumber, foundBracketEndOffset + 1), bracketIsOpen ? 1
        /* Open */
        : -1
        /* Close */
    ...
    if (startOffset < endOffset) {
      ...
      this._nextBuff.push(new Token(new Range(lineNumber, startOffset + 1, lineNumber, endOffset + 1), 0
      /* None */
      , null));
    }
...
function () {
  ...
  TokenTreeBuilder.prototype._accept = function (condt) {
    ...
    if (!accepted) {
      ...
      this._currentToken = token; //			console.log('accepted: ' + token.__debugContent);
    }
  ...
  TokenTreeBuilder.prototype._line = function () {
    var node = new NodeList();
    var lineNumber; // capture current linenumber
    ...
    while (this._peek(function (info) {
      return info.range.startLineNumber === lineNumber;
    }) && node.append(this._token() || this._block())) {// all children that started on this line
    }
  ...
  TokenTreeBuilder.prototype._token = function () {
    if (!this._accept(function (token) {
      return token.bracket === 0
      /* None */
      ;
    })) {
      return null;
  ...
  TokenTreeBuilder.prototype._block = function () {
    ...
    while (bracket.elements.append(this._line())) {// inside brackets
    }
