...
export var editorWordHighlight = registerColor('editor.wordHighlightBackground', {
  ...
  light: '#57575740',
  hc: null
}, nls.localize('wordHighlight', 'Background color of a symbol during read-access, like reading a variable. The color must not be opaque to not hide underlying decorations.'), true);
export var editorWordHighlightStrong = registerColor('editor.wordHighlightStrongBackground', {
  dark: '#004972B8',
  light: '#0e639c40',
  hc: null
}, nls.localize('wordHighlightStrong', 'Background color of a symbol during write-access, like writing to a variable. The color must not be opaque to not hide underlying decorations.'), true);
...
export var overviewRulerWordHighlightForeground = registerColor('editorOverviewRuler.wordHighlightForeground', {
  ...
  light: '#A0A0A0CC',
  hc: '#A0A0A0CC'
}, nls.localize('overviewRulerWordHighlightForeground', 'Overview ruler marker color for symbol highlights. The color must not be opaque to not hide underlying decorations.'), true);
export var overviewRulerWordHighlightStrongForeground = registerColor('editorOverviewRuler.wordHighlightStrongForeground', {
  dark: '#C0A0C0CC',
  light: '#C0A0C0CC',
  hc: '#C0A0C0CC'
}, nls.localize('overviewRulerWordHighlightStrongForeground', 'Overview ruler marker color for write-access symbol highlights. The color must not be opaque to not hide underlying decorations.'), true);
...
function () {
  ...
  WordHighlighter.prototype._getSortedHighlights = function () {
    ...
    return this._decorationIds.map(function (id) {
  ...
  WordHighlighter.prototype.moveNext = function () {
    ...
    var index = firstIndex(highlights, function (range) {
  ...
  WordHighlighter.prototype._stopAll = function () {
    this._lastWordRange = null; // Remove any existing decorations
  ...
  WordHighlighter.prototype._run = function () {
    var _this = this; // no providers for this model
    ...
    var workerRequestIsValid = this._lastWordRange && this._lastWordRange.equalsRange(currentWordRange); // Even if we are on a different word, if that word is in the decorations ranges, the request is still valid
    // (Same symbol)
    ...
    if (workerRequestIsValid) {
      ...
      this.workerRequestCompleted = false;
      this.workerRequest = createCancelablePromise(function (token) {
        return getOccurrencesAtPosition(_this.model, _this.editor.getPosition(), token);
      });
