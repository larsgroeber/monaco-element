...
function (_super) {
  ...
  function ParameterHintsModel(editor) {
    ...
    _this.throttledDelayer = new RunOnceScheduler(function () {
  ...
  ParameterHintsModel.prototype.trigger = function (delay) {
    if (delay === void 0) {
      delay = ParameterHintsModel.DELAY;
    }
  ...
  ParameterHintsModel.prototype.doTrigger = function () {
    ...
    this.provideSignatureHelpRequest = createCancelablePromise(function (token) {
      return provideSignatureHelp(_this.editor.getModel(), _this.editor.getPosition(), token);
    });
...
function () {
  ...
  ParameterHintsWidget.prototype.show = function () {
    ...
    this.visible = true;
    TPromise.timeout(100).done(function () {
  ...
  ParameterHintsWidget.prototype.render = function () {
    ...
    if (activeParameter) {
      var labelToAnnounce = activeParameter.label; // Select method gets called on every user type while parameter hints are visible.
  ...
  ParameterHintsWidget.prototype.renderParameters = function (parent, signature, currentParameter) {
    ...
    for (var i = signature.parameters.length - 1; i >= 0; i--) {
      ...
      dom.prepend(parent, element); // parameter part
  ...
  ParameterHintsWidget.prototype.next = function () {
    ...
    var last = this.currentSignature % length === length - 1; // If there is only one signature, or we're on last signature of list
