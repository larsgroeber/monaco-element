...
export function getCodeActions(model, rangeOrSelection, trigger, token) {
  ...
  var promises = CodeActionProviderRegistry.all(model).map(function (support) {
    return asWinJsPromise(function (token) {
      return support.provideCodeActions(model, rangeOrSelection, codeActionContext, token);
    }).then(function (providedCodeActions) {
      if (!Array.isArray(providedCodeActions)) {
        return [];
      }

      return providedCodeActions.filter(function (action) {
        return isValidAction(trigger && trigger.filter, action);
      });
    }, function (err) {
  ...
  return Promise.all(promises).then(flatten).then(function (allCodeActions) {
    return mergeSort(allCodeActions, codeActionsComparator);
  });
}
