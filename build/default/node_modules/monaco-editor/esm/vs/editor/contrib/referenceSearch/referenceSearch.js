...
function (_super) {
  ...
  ReferenceAction.prototype.run = function (accessor, editor) {
    ...
    var references = createCancelablePromise(function (token) {
      return provideReferences(model, range.getStartPosition(), token).then(function (references) {
...
var showReferencesCommand = function (accessor, resource, position, references) {
  ...
  return codeEditorService.openCodeEditor({
    ...
    return TPromise.as(controller.toggleWidget(new Range(position.lineNumber, position.column, position.lineNumber, position.column), createCancelablePromise(function (_) {
      return Promise.resolve(new ReferencesModel(references));
    }), defaultReferenceSearchOptions)).then(function () {
      return true;
    });
  });
}; // register commands
