...
function () {
  ...
  SuggestModel.prototype._updateTriggerCharacters = function () {
    ...
    if (this._editor.getConfiguration().readOnly || !this._editor.getModel() || !this._editor.getConfiguration().contribInfo.suggestOnTriggerCharacters) {
      return;
    }
    ...
    this._triggerCharacterListener = this._editor.onDidType(function (text) {
      ...
      if (supports) {
        ...
        }, Boolean(_this._completionModel), values(supports), items);
      }
    });
  };
  ...
  SuggestModel.prototype._onCursorChange = function (e) {
    ...
    if (!e.selection.isEmpty() || e.reason !== CursorChangeReason.NotSet || e.source !== 'keyboard' && e.source !== 'deleteLeft') {
  ...
  SuggestModel.prototype.trigger = function (context, retrigger, onlyFrom, existingItems) {
    ...
    var ctx = new LineContext(model, this._editor.getPosition(), auto); // Cancel previous requests, change state & update UI
    ...
    if (context.triggerCharacter) {
      ...
      suggestCtx = {
        triggerKind: SuggestTriggerKind.TriggerForIncompleteCompletions
      };
    } else {
      suggestCtx = {
        triggerKind: SuggestTriggerKind.Invoke
      };
    }

    this._requestPromise = createCancelablePromise(function (token) {
      return provideSuggestionItems(model, _this._editor.getPosition(), _this._editor.getConfiguration().contribInfo.suggest.snippets, onlyFrom, suggestCtx, token);
    });
  ...
  SuggestModel.prototype._onNewContext = function (ctx) {
    ...
    if (ctx.column > this._context.column && this._completionModel.incomplete.size > 0 && ctx.leadingWord.word.length !== 0) {
      ...
      }, true, values(incomplete), adopted);
    } else {
