...
import { MenuId } from '../../../platform/actions/common/actions.js'; // --- selection state machine
...
function () {
  ...
  SmartSelectController.prototype.run = function (forward) {
    ...
    var model = this.editor.getModel(); // forget about current state
    ...
    if (!this._state) {
      promise = this._tokenSelectionSupport.getRangesToPosition(model.uri, selection.getStartPosition()).then(function (elements) {
        ...
        _this._state = editorState; // listen to caret move and forget about state
  ...
  SmartSelectController = __decorate([__param(1, IInstantiationService)], SmartSelectController);
